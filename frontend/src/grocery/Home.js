...
const Home = () => {
  ...
  useEffect(() => {
    /* get all item */
    axios
      .get("http://localhost:3001/grocery/getAll")
      .then((res) => {
  ...
  const handleKeyPress = (e) => {
    if (e.key === "Enter") {
      e.preventDefault();
      axios
        .post("http://localhost:3001/grocery/add", {
          groceryItem: item,
          isPurchased: false,
        })
        .then((result) => {
  ...
  const handleUpdate = (id) => {
    axios
      .put(`http://localhost:3001/grocery/updatePurchaseStatus/${id}`, {
        isPurchased: true,
      })
      .then((result) => {
        console.log(result);
  ...
  const handleDelete = (id) => {
    axios
      .delete(`http://localhost:3001/grocery/deleteGroceryItem/${id}`)
      .then((result) => {
        console.log(result);
        setStalker(true);
      })
